...
export default class WorkItem extends React.Component {
    ...
    showContent() {
        setTimeout(() => {
            this.child.enable()
            document.getElementById(`work-item-${this.props.index}`).classList.add("blue-shadow");
        }, 800)
    }
    ...
    render() {
        ...
        if (this.props.type === "slider") {
            return (
                  <Link  to={`/work/${this.props.name}`} rel="noopener noreferrer">
                    <Tilt options={{ scale: 1, max: 10 }}>
                        <Item className="blue-shadow">
                            ...
                            <DesktopContent name={this.props.name} category={this.props.category} ref={(cd) => this.child = cd} type={this.props.type} />
                        </Item>
                    </Tilt>
                </Link>
            )
